    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                Console.WriteLine("Значение компоненты вектора с индексом 1: {0}", vector4.GetDimention(1));
                vector4.SetDimention(4, 6.7);
                Console.WriteLine("Устанавливаем компонент вектора с индексом 4: {0}", vector4.GetDimention(4));
                ...
                Console.WriteLine("Получившийся вектор: ({0})", vector2.ToString());
                Console.WriteLine("Вычитаем c помощью статического метода из вектора ({0}) вектор ({1}).", vector2.ToString(), vector4.ToString());
                Vector vector6 = Vector.ScalarProduct(vector2, vector4);
                Console.WriteLine("Получившийся вектор: ({0})", vector6.ToString());
